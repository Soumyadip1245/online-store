import { Info, Products } from './util'
import axios from 'axios'
async function getData(searchWord: string) {
    const html = await axios.get(`https://api.scraperapi.com/structured/google/shopping?api_key=${process.env.TOKEN}&query=${searchWord + " myntra"}&country_code=in`)
    return html.data
}
export async function myntraData(searchWord: string): Promise<Info> {
    try {
